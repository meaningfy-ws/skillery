# vault-project-lifecycle Specification

## Purpose
A vault project from charter to archive: the charter gate on creation, one-project session start,
capture that keeps hedges and never invents, and closing with an archive move only after a yes.
## Requirements
