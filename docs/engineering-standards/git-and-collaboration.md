...
- **Branches:** `<type>/<ticket-id>/<short-label>`; let the ticketing system generate the name.
- **Workflow:** GitFlow for client projects, a lighter flow internally; `pull --rebase` on shared
  branches; rebase to incorporate base changes (merge if numerous); notify peers before a
