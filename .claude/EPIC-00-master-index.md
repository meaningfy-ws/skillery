...
| **DEC-2** | **Full OpenSpec adoption** via a **forked `meaningfy` schema** (custom `EPIC.md` + `PLAN.md` artifacts, per-artifact `rules:`), `/opsx:*` commands, and `openspec validate --strict` in CI. OpenSpec is the artifact-lifecycle engine; Meaningfy skills do elicitation, semantic clarity, BDD, and TDD execution. Also define Meaningfy-specific OpenSpec **workflows/profiles** (combos of `/opsx` commands). | OpenSpec's schema-fork model bends the framework to our artifacts; no parallel tooling. |
...
| **DEC-9** | **Spec stewardship folds into `epic-planning` + spine conventions** (no dedicated stewardship skill). `epic-planning` owns the seed→EPIC→PLAN authoring flow and the EPIC↔change lifecycle; `/opsx:archive` + the spine doc + AGENTS conventions cover grooming/archiving/validate. | Leanest; YAGNI. |
...
| 3 | **PLAN = `tasks.md` (+ `design.md`)**, clarity-gated ≥9/10 | `openspec/changes/<id>/` | derived from the EPIC |
